...
export default function ProjectFilmPlayer({ film }: { film: ProjectFilm }) {
        ...
        <video
          ...
          onTimeUpdate={(event) => {
            ...
            if (!video.paused && video.readyState >= 3)
              setMode(current => current === 'loading' ? 'playing' : current)
          }}
          ...
          onPlay={(event) => { guardPlayback(event.currentTarget) }}
          onPlaying={(event) => {
