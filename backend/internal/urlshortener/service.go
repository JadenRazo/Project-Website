...
func (s *Service) GetURLStats(c *gin.Context) {
    ...
    s.db.Model(&entity.URLClick{}).Where("url_id = ?", shortenedURL.ID).Count(&totalClicks)
    s.db.Model(&entity.URLClick{}).Where("url_id = ?", shortenedURL.ID).Distinct("ip_address").Count(&uniqueClicks)
    ...
    s.db.Model(&entity.URLClick{}).Select("country, COUNT(*) as count").
        Where("url_id = ? AND country != ''", shortenedURL.ID).
        Group("country").Order("count DESC").Limit(10).Find(&countryStats)
    ...
    s.db.Model(&entity.URLClick{}).Select("DATE(clicked_at) as date, COUNT(*) as count").
        Where("url_id = ? AND clicked_at >= ?", shortenedURL.ID, time.Now().AddDate(0, 0, -30)).
    ...
    s.db.Model(&entity.URLClick{}).Select("referer, COUNT(*) as count").
        Where("url_id = ? AND referer != ''", shortenedURL.ID).
    ...
    s.db.Where("url_id = ?", shortenedURL.ID).Order("clicked_at DESC").Limit(20).Find(&recentClicks)
...
func (s *Service) GetUserURLs(c *gin.Context) {
    ...
    for _, url := range urls {
        ...
        s.db.Model(&entity.URLClick{}).Where("url_id = ?", url.ID).Count(&totalClicks)
        s.db.Model(&entity.URLClick{}).Where("url_id = ?", url.ID).Distinct("ip_address").Count(&uniqueClicks)
...
func (s *Service) recordClick(urlID uuid.UUID, c *gin.Context) {
    ...
    click := &entity.URLClick{
        ...
        UserAgent: userAgent,
        Referer:   referer,
        Country:   "", // TODO: Implement GeoIP lookup
