...
if [ -f "$MIGRATION_CMD" ]; then
    ...
    cd "${BACKEND_DIR}"
    go run "$MIGRATION_CMD" up -env "$ENVIRONMENT" || {
        echo -e "${RED}Warning: Database migrations had errors but we will continue.${NC}"
    }
else
    echo -e "${YELLOW}Migration command not found at ${MIGRATION_CMD}. Skipping migrations.${NC}"
fi
