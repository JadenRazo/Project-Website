...
run_database_migrations() {
    ...
    if [ -f "schema.sql" ]; then
        verbose "Database schema file found at backend/schema.sql"
        log "To initialize database, run: psql -d project_website -f backend/schema.sql"
    else
        warning "No schema.sql found - database initialization required"
    fi
...
show_help() {
    ...
    echo "  -u, --update-stats  Update code statistics after startup"
    ...
    echo "  $0 --update-stats   # Update code statistics after startup"
...
update_code_statistics() {
    log "Updating code statistics..."
    
    # Wait a moment for services to stabilize
    sleep 3
    
    # Try to update code statistics via API
    if command -v curl &> /dev/null; then
        verbose "Using curl to update code statistics"
        local response
        if response=$(curl -s -X POST -H "Content-Type: application/json" \
                     "http://localhost:$API_PORT/api/v1/code/stats/update" 2>/dev/null); then
            if echo "$response" | grep -q "success\|totalLines"; then
                success "Code statistics updated successfully"
                verbose "API Response: $response"
            else
                warning "Code statistics update may have failed"
                verbose "API Response: $response"
            fi
        else
            warning "Failed to connect to API for code statistics update"
            warning "You can manually update stats later via the DevPanel"
        fi
    else
        warning "curl not found - cannot update code statistics automatically"
        warning "You can manually update stats via the DevPanel"
    fi
}
...
main() {
    ...
    # Update code statistics if requested
